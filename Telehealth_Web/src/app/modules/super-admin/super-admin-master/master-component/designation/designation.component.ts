...
export class DesignationComponent implements OnInit {
  ...
  constructor(
    ...
    this.editDesignationForm = this.fb.group({
      ...
      designation_arabic: ["", [Validators.required]],
  ...
  onSortData(column: any) {
    ...
    this.getAllDesignationList(`${column}:${this.sortOrder}`);
  }
  ...
  ngOnInit(): void {
    ...
    this.getAllDesignationList(`${this.sortColumn}:${this.sortOrder}`);
    // setTimeout(() => {
  ...
  excleSubmit() {
    ...
    // uploadExcelMedicine
    this._superAdminService.uploadExcelDesignationList(formData).subscribe(
      (res: any) => {
  ...
  downLoadExcel() {
    ...
    link.setAttribute("href", "assets/doc/DesignationFile.xlsx");
    link.setAttribute("download", `DesignationFile.xlsx`);
  ...
  exportSpeciality() {
    ...
    this.pageSize = 0;
    this.loader.start();
    this._superAdminService.allDesignationListforexport(this.page, this.pageSize, this.searchText)
      .subscribe((res) => {
        ...
        if (result.status == true) {
          ...
          var fileName = 'Designation_List.xlsx';
  ...
  getAllDesignationList(sort: any = '') {
    let reqData = {
      page: this.page,
      limit: this.pageSize,
      userId: this.userId,
    ...
    this._superAdminService.DesignationLists(reqData).subscribe((res) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
  ...
  addDesignationhandle() {
    ...
    let reqData = {
      designationArray: this.designationForm.value.designations,
      added_by: this.userId,
    };


    this._superAdminService.addDesignation(reqData).subscribe((res: any) => {
      let encryptedData = { data: res };
  ...
  updatedesignation() {
    ...
    this._superAdminService
      .updateDesignatonApi(this.editDesignationForm.value)
      .subscribe((res: any) => {
        let encryptedData = { data: res };
        let response = this._coreService.decryptObjectData(encryptedData);
  ...
  deleteDesignationHandle(isDeleteAll: any = "") {
    this.loader.start();
    let reqData = {
    ...
    if (isDeleteAll === "all") {
      ...
      reqData.designationId = this.selectedSpecialities;
    }

    this.service.deleteDesignation(reqData).subscribe((res: any) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
      if (response.status) {
        this.loader.stop();
  ...
  addnewSpeciality() {
    this.designations.push(this.newSpecialityForm());
  }
  ...
  openVerticallyCenterededitspecialityservice(
    ...
    this.editDesignationForm.patchValue({
      ...
      designation_arabic: data.designation_arabic,
  ...
  //delete popup
  openVerticallyCenteredsecond(deletePopup: any, designationId: any) {
