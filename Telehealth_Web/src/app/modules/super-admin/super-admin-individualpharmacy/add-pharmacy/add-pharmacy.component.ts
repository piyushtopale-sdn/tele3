...
export class AddPharmacyComponent {
  ...
  ngOnInit(): void {
    ...
    this.pharmacyID = paramId;

    if (this.pharmacyID !== null) {
      this.getProfile(this.pharmacyID)
  ...
  public profileFields: FormGroup = new FormGroup(
    {
      ...
      licence_details: new FormGroup({
        id_number: new FormControl("", [Validators.required,Validators.pattern(/^[a-zA-Z0-9]{6,}$/),Validators.maxLength(20)]),
        expiry_date: new FormControl("",[this.coreService.LicenseDateValidator()]),
  ...
  public async createProfile(): Promise<void> {
    ...
    const profileRequest = {
      for_portal_user: this.pharmacyID ? this.pharmacyID : "",
      address: data?.address,
      email: data.email?.toLowerCase(),
      pharmacy_name: data.pharmacy_name,
      pharmacy_name_arabic: data.pharmacy_name_arabic,
      slogan: data.slogan,
      main_phone_number: data.main_phone_number,
      additional_phone_number: data.additional_phone_number,
      about_pharmacy: data.about_pharmacy,
      profile_picture: data.profile_picture,
      // profile_picture:this.profile_picture,
      licence_details: {
        licence_picture: data.licence_details.licence_picture,
        // license_picture:data.licence_details.license_picture,
    ...
    this.sadminService.createPharmacyProfile(profileRequest).subscribe({
      next: (result) => {
        let response = this.coreService.decryptObjectData({ data: result });
        if (response.status == true) {
          ...
          if(this.pharmacyID !== null){
            ...
            this.route.navigate([`/super-admin/pharmacy/permission/${id}`]);
          }
  ...
  getProfile(userId: any) {
    return new Promise(async (resolve, reject) => {
      this.sadminService.viewProfileById(userId).subscribe({
        next: (result) => {
          let response = this.coreService.decryptObjectData({ data: result });
          ...
          this.pharmacyPicUrl = response?.data?.adminData?.pharmacy_picture_signed_urls;

          this.license_picture = response?.data?.adminData?.licencePicSignedUrl
          this.profile_picture = response?.data?.adminData?.profile_picture_signed_url;

          response?.data?.adminData?.pharmacy_picture.forEach((element) => {          //already added pictures
            this.docArray.push(element);
          });
          this.profileData = {
            pharmacy_name: response?.data?.adminData?.pharmacy_name,
            pharmacy_name_arabic: response?.data?.adminData?.pharmacy_name_arabic,
