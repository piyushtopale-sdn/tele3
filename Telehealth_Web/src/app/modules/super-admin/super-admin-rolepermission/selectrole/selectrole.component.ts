...
export class SelectroleComponent implements OnInit {
  ...
  constructor(
    ...
    private _pharmacy: SuperAdminService,
  ...
  private getAllStaff() {
    this._pharmacy.gettAllStaffListing({ for_user: this.userID }).subscribe({
      next: (result: any) => {
        ...
        // this.staffList = this.staffList.filter((data) => {
        //   return (data && data?.superadmin_id?._id)
        // })
      decryptedData?.body.map((curentval, index:any) => { 
        if(curentval?.superadmin_id !== null){
          if(this.staffList.indexOf({
            label:curentval?.superadmin_id?.fullName,
            value: curentval?.superadmin_id?._id,
          })== -1) {
            this.staffList.push({
              label:curentval?.superadmin_id?.fullName,
              value: curentval?.superadmin_id?._id,
    ...
    this._pharmacy.getUserMenus(params).subscribe(
      (res: any) => {
        ...
        for (const data of decryptedData.body) {
          ...
          if(data.menu_id.name != "Staff Management" && data.menu_id.name !="Role Permission"){
  ...
  submenudatabyuser() {
    this._pharmacy.submenudatabyuser({ user_id: this.staffID, module_name: "superadmin" }).subscribe((result: any) => {
