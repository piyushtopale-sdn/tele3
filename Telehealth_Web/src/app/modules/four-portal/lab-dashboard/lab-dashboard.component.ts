...
export class LabDashboardComponent implements OnInit {
  ...
  initializeChartData(completedCounts: any = '', cancelledCounts: any = '', underProcessedCounts: any = '', labels: any = '') {
    this.lineChartData = {
      labels: labels,
      datasets: [
        {
  ...
  dashboardGraph() {
    ...
    this.fourPortalService.dashboardGraph(data).subscribe((res: any) => {
      ...
      if (response.status) {
        ...
        const underProcessedCounts = underProcessedData.map(item => item.count);
        this.initializeChartData(completedCounts, cancelledCounts, underProcessedCounts, labels);
        const allCounts = [
          ...completedCounts,
          ...cancelledCounts,
