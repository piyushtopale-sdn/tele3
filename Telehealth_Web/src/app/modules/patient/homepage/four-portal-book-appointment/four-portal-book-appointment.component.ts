...
@Component({
  selector: 'app-four-portal-book-appointment',
  templateUrl: './four-portal-book-appointment.component.html',
  styleUrls: ['./four-portal-book-appointment.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class FourPortalBookAppointmentComponent implements OnInit {
  ...
  portal_Details() {
    ...
    this.fourPortalService.fourPortalDetails(param).subscribe({
      next: async (res) => {
        ...
        this.portaldetailsData = await JSON.parse(
  ...
  async book_Appointment() {
    ...
    let param = {
      ...
      patientDetails: {
        patientId: this.patient_details.value.subPatient_id ?  this.patient_details.value.subPatient_id : this.loginuser_id
        ...
        patientImage :  this.adminData?.profile_pic,
          ...
          if (res === true) {
            this._PatientService.fourPortal_bookAppointment(param).subscribe({
              next: (res) => {
                ...
                let appointmentdetails = { appointment_id: result.body._id , portal_type:this.portal_type };

                if (result.status) {
                  this.router.navigate(
                    ["/patient/homepage/portal-view-appointment"],
                    {
  ...
  goBack() {
    this.router.navigate([`patient/homepage/list/${this.portal_type}`])
  }
