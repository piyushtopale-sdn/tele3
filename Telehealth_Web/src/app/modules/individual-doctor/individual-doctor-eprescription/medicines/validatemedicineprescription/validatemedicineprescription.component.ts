...
export class ValidatemedicineprescriptionComponent implements OnInit, AfterViewInit {
    ...
    this.indiviualDoctorService
      ...
      .subscribe((res) => {
        ...
        if (response.status) {
          ...
          //For MEdicine Dosage
          this.allDosages = data?.dosageData;
          data?.dosageData.forEach(async (element) => {
            let obj = {
            ...
            let result = this.listMedicineDosages.filter((s) =>
              s?.medicineName.includes(element.medicineName)
            );
            if (result.length === 0) {
  ...
  downloadPreview() {
    const div = this.previewdiv.nativeElement;
  
    // Use html2canvas to capture the content of the previewdiv
    html2canvas(div, {
      scale: 2, // Higher resolution for better output
      useCORS: true, // Allow cross-origin content if needed
    }).then((canvas) => {
      ...
      // Define A4 paper size (210mm x 297mm)
      ...
      // Scale the image to fit A4 page size while maintaining the aspect ratio
      if (aspectRatio > pdfAspectRatio) {
        // Image is wider than the PDF, so scale by width
        finalWidth = pdfWidth;
        finalHeight = pdfWidth / aspectRatio;
      } else {
        // Image is taller than the PDF, so scale by height
      ...
      // Calculate the position to center the image in the PDF page
      const xOffset = (pdfWidth - finalWidth) / 2;
      const yOffset = (pdfHeight - finalHeight) / 2;
  
      // Add the image to the PDF and set it in the center
