...
@Component({
  selector: 'app-validate-radiotest',
  templateUrl: './validate-radiotest.component.html',
  styleUrls: ['./validate-radiotest.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ValidateRadiotestComponent {
  signatureImg: string = "";
    ...
    this.indiviualDoctorService
      ...
      .subscribe((res) => {
        ...
        if (response.status) {
          ...
          this.allDosages = data?.radiologyTest;
          data?.radiologyTest.forEach(async (element) => {
            let obj = {
              _id: element?.radiologyTestId,
              testName: element?.radiologyTestName,
            };

            let result = this.listMedicineDosages.filter((s) =>
              s?.testName.includes(element.radiologyTestName)
            );
            if (result.length === 0) {
