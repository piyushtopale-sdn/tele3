  ...
  try {
    ...
    if (checkUser?.role === "PHARMACY_STAFF") {
      const data = await PortalUser.findOne({
        staff_createdBy: checkUser?.staff_createdBy,
      });
    ...
    let filter = { for_user: userId, is_delete: "No" }; // Update the filter to include 'is_delete: "No"'
    let roles = "";
    if (limit == undefined) {
      limit = 0;
    }
    ...
    if (limit == 0) {
      roles = await Role.find(filter).sort(sortingarray).exec();
    } else {
...
export const delete_role = async (req, res) => {
  try {
    ...
    await Role.findByIdAndUpdate({ _id: req.body.id }, role, {
      upsert: false,
      new: true,
    });
    handleResponse(req, res, 200, {
