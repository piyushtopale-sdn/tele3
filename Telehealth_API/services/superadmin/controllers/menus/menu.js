...
const all_menus = async (req, res) => {
    try {
        const menus = await Menus.find({disable_menu : false, status: { $eq: 1 }, module_type: req.query.module_type }).sort({ 'menu_order': 1 });
        handleResponse(req, res, 200, {
            status: true,
            body: menus,
            message: "super-admin fetched all menus",
            errorCode: null,
        })
    } catch (error) {
        handleResponse(req, res, 500, {
...
const all_submenus = async (req, res) => {
    try {
        ...
        handleResponse(req, res, 500, {
            status: false,
            body: null,
            message: "Failed to fetch all submenus",
            errorCode: "INTERNAL_SERVER_ERROR",
        })
    }
