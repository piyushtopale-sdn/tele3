...
import VitalThreshold from "../../models/vitalsthreshold/vitalThresholdMaster";

export const addNewvitalsThreshold = async (req, res) => {
   
        const { vitalsType, rangeData } = req.body
        const existVital = await VitalThreshold.findOne({ vitalsType: vitalsType , isDeleted: false});
        ...
        if (vitalsType === "BLOOD_PRESSURE") {
            ...
            try {
                const addData = new VitalThreshold(addObject);
                await addData.save();
...
export const getNewvitalsThreshold = async (req, res) => {
    try {
        ...
        const result = await VitalThreshold.aggregate(pipeline);
...
export const updateNewvitalsThreshold = async (req, res) => {
    try {
        const { id, vitalsType, rangeData } = req.body;         //API Fixed AYAN
        ...
        if (!vitalsType || !Array.isArray(rangeData) || rangeData.length === 0) {
            return sendResponse(req, res, 400, {
                status: false,
                body: null,
                message: "Vitals type and valid range data are required",
                errorCode: "INVALID_INPUT",
            });
        }

        const isValidRangeData = rangeData.every((item) =>
            item.gender && item.age !== undefined && item.high !== undefined &&
            item.low !== undefined && item.criticalHigh !== undefined &&
            item.criticalLow !== undefined && item.unit
        );
        ...
        if (!isValidRangeData) {
            return sendResponse(req, res, 400, {
                status: false,
                body: null,
                message: "Invalid range data structure",
                errorCode: "INVALID_RANGE_DATA",
            });
        }
        const updateResult = await VitalThreshold.findOneAndUpdate(
            { _id: id },  
            { $set: { vitalsType: vitalsType, referenceRange: rangeData } },  
            { new: true } 
        );
...
export const updateNewvitalsThresholdByAction = async (req, res) => {
    try {
        ...
        await VitalThreshold.updateMany(
            {
                _id: { $in: id },
            },
            {
        ...
        sendResponse(req, res, 200, {
            status: true,
            body: null,
            message: `Vitals threshold ${
                actionName === 'isDeleted' ? 'deleted' : 'updated'
            } successfully`,
            errorCode: null,
        });
    } catch (error) {
